...
export class DonBaoHanhService {
  ...
  protected convertDateFromClient(donBaoHanh: IDonBaoHanh): IDonBaoHanh {
    return Object.assign({}, donBaoHanh, {
      ngaykhkb: donBaoHanh.ngaykhkb?.isValid() ? donBaoHanh.ngaykhkb.toJSON() : undefined,
      ngayTraBienBan: donBaoHanh.ngayTraBienBan?.isValid() ? donBaoHanh.ngayTraBienBan.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.ngaykhkb = res.body.ngaykhkb ? dayjs(res.body.ngaykhkb) : undefined;
      res.body.ngayTraBienBan = res.body.ngayTraBienBan ? dayjs(res.body.ngayTraBienBan) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((donBaoHanh: IDonBaoHanh) => {
        donBaoHanh.ngaykhkb = donBaoHanh.ngaykhkb ? dayjs(donBaoHanh.ngaykhkb) : undefined;
        donBaoHanh.ngayTraBienBan = donBaoHanh.ngayTraBienBan ? dayjs(donBaoHanh.ngayTraBienBan) : undefined;
      });
    }
    return res;
  }
